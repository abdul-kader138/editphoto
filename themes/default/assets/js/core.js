...
$(document).ready(function () {
    ...
    $('.input-pop').popover({
        placement: 'top', html: true, trigger: 'hover', container: 'body',
        content: function () {
            return $(this).attr('data-tip');
        },
        title: function () {
...
$('body').on('click', '.invoices_link td:not(:first-child, :nth-child(2), :last-child)', function() {
    $('#myModal').modal({remote: site.base_url + 'invoices/modal_view/' + $(this).closest('tr').attr('id')});
    $('#myModal').modal('show');
});
...
function setAllParamsList(objs) {
    setServiceTypeList(objs);
    setComplexityTypeList(objs);
    setAddOnsTypeList(objs);
    setDeliveryFormatTypeList(objs);
    setDeliveryCostTypeList(objs);
}

function setServiceTypeList(obj) {
    var date_id = obj.id;
    var id = date_id.split("_");
    var service_id = 'service_id_' + id[2];
    $(this).addClass('ui-autocomplete-loading');
    $.ajax({
        type: "get", async: false,
        url: site.base_url + "invoices/getServiceType/",
        dataType: "json",
        data: {},
        success: function (data) {
            $(this).removeClass('ui-autocomplete-loading');
            $('#sel_service_id_' + id[2]).remove();
            if (data) {
                var opt = $("<select id='sel_service_id_" + id[2] + "' name='service_id[]' class='form-control select' />");
                $.each(data, function () {
                ...
                iziToast.show({
                    ...
                    color: 'yellow',
                    progressBar: true,
                    message: 'Still,Service Type is not available.'
                });
            }
        }
    });
...
function setComplexityTypeList(obj) {
    var date_id = obj.id;
    var id = date_id.split("_");
    var complexity_id = 'complexity_id_' + id[2];
    $(this).addClass('ui-autocomplete-loading');
    $.ajax({
        type: "get", async: false,
        url: site.base_url + "invoices/getComplexityType/",
        dataType: "json",
        data: {},
        success: function (data) {
            $(this).removeClass('ui-autocomplete-loading');
            $('#sel_complexity_id_' + id[2]).remove();
            if (data) {
                var opt = $("<select id='sel_complexity_id_" + id[2] + "' name='complexity_id[]' class='form-control select' />");
                $.each(data, function () {
                    $("<option />", {
                        value: this.id,
                        text: (this.name + ' - ' + this.price + ' per image')
                    }).appendTo(opt);
                });
                $('#' + complexity_id).append(opt);
            } else {
                iziToast.show({
                    title: 'Warning:',
                    color: 'yellow',
                    progressBar: true,
                    message: 'Still,Complexity Type is not available.'
                });
            }
        }
    });
...
function setAddOnsTypeList(obj) {
    var date_id = obj.id;
    var id = date_id.split("_");
    var addOns_id = 'addOns_id_' + id[2];

    $.ajax({
        type: "get", async: false,
        url: site.base_url + "invoices/getAddOnsType/",
        dataType: "json",
        data: {},
        success: function (data) {
            $(this).removeClass('ui-autocomplete-loading');
            $('#sel_addOns_id_' + id[2]).remove();
            if (data) {
                var opt = $("<select id='sel_addOns_id_" + id[2] + "' name='addOns_id[]' class='form-control select' />");
                $.each(data, function () {
                    $("<option />", {
                        value: this.id,
                        text: (this.name + ' - ' + this.price + ' per image')
                ...
                $('#' + addOns_id).append(opt);
            } else {
                iziToast.show({
                    title: 'Warning:',
                    color: 'yellow',
                    progressBar: true,
                    message: 'Still,Add Ons is not available.'
                });
            }
        }
    });
...
function setDeliveryFormatTypeList(obj) {
    var date_id = obj.id;
    var id = date_id.split("_");
    var deliveryFormat_id = 'deliveryFormat_id_' + id[2];

    $.ajax({
        type: "get", async: false,
        url: site.base_url + "invoices/getDeliveryFormatType/",
        dataType: "json",
        data: {},
        success: function (data) {
            $(this).removeClass('ui-autocomplete-loading');
            $('#sel_deliveryFormat_id_' + id[2]).remove();
            if (data) {
                var opt = $("<select id='sel_deliveryFormat_id_" + id[2] + "' name='deliveryFormat_id[]' class='form-control select' />");
                $.each(data, function () {
                    $("<option />", {
                        value: this.id,
                        text: (this.name)
    ...
    function setDeliveryCostTypeList(obj) {
        var date_id = obj.id;
        var id = date_id.split("_");
        var deliveryCost_id = 'deliveryCost_id_' + id[2];

        $.ajax({
            type: "get", async: false,
            url: site.base_url + "invoices/getDeliveryCostType/",
            dataType: "json",
            data: {},
            success: function (data) {
                $(this).removeClass('ui-autocomplete-loading');
                $('#sel_deliveryCost_id_' + id[2]).remove();
                if (data) {
                    var opt = $("<select id='sel_deliveryCost_id_" + id[2] + "' name='deliveryCost_id[]' class='form-control select' />");
                    $.each(data, function () {
                        $("<option />", {
                            value: this.id,
                            text: (this.name + ' - ' + this.price + ' per image')
