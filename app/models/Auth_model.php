...
class Auth_model extends CI_Model
{
    ...
    public function updateStaffLogin($id, $data = array())
    {
        ...
        $this->db->where('id', $id);
        if ($this->db->update("user_logins", $loginData)) {
            return true;
        }
        return false;
    }
    ...
    public function updateStaffLogout($id, $data = array())
    {
        ...
        $this->db->where('id', $id);
        if ($this->db->update("user_logouts", $logoutData)) {
            return true;
        }
        return false;
    }
    ...
    public function userLogouts()
    {
        ...
        $this->db->where('login', $user_identity);
    ...
    public function usersProductGroups($upg)
    {
        If ($this->getUPG($upg['user_id'], $upg['product_group_id'])) {
            ...
            $this->db->insert('users_product_groups', $upg);
            return true;
        }
        return false;
    }
    ...
    public function getAllUserProductGroups($id)
    {
        ...
        $this->db->select("product_groups.id as id, name, (CASE WHEN UPG.pgi IS NOT NULL 
       THEN UPG.percent
...
END) as percent")
    ...
    public function punchout($dataupdate = array())
    {
        ...
        if ($this->db->update('workingtime', $dataupdate)) {
            return true;
        }
        return false;
    }
    ...
    public function reloginConfirmation($identity, $password)
    {
        ...
        if ($query->num_rows() === 1) {
            ...
            return ($password === true) ? $user : false;
        }
